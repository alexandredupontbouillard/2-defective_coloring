...
void Cplex_Noproj_algo::create_MIP(C_Graph *GG,int maxNodes){
	...
	for (k=0;k<G->nb_links;k++){
		...
		for(list<C_link*>::iterator itk = G->V_nodes[G->V_links[k]->v1].L_adjLinks.begin() ; itk != G->V_nodes[G->V_links[k]->v1].L_adjLinks.end() ; itk++){
			if((*itk)->return_other_extrem(G->V_links[k]->v1) != G->V_links[k]->v2)
				cst+=y[edgeNumber( G->V_links[k]->v1 , G->V_links[k]->v2)];
		
		}
		for(list<C_link*>::iterator itk = G->V_nodes[G->V_links[k]->v2].L_adjLinks.begin() ; itk != G->V_nodes[G->V_links[k]->v2].L_adjLinks.end() ; itk++){
			if((*itk)->return_other_extrem(G->V_links[k]->v2) != G->V_links[k]->v1)
				cst+=y[edgeNumber( G->V_links[k]->v1 , G->V_links[k]->v2)];
...
int Cplex_Noproj_algo::edgeNumber(int v1 , int v2){

	return G->edgeNumber(v1,v2);

}

void Cplex_Noproj_algo::set_objective_coefficient(const vector<SCIP_Real>& obj_coeff, const vector<SCIP_Real>& coeff_additional){
	int i;
...
*/
	...
	for( i=0; i< G->nb_links ; i++){
		...
		cst+= y[i] * (abs(obj_coeff[G->V_links[i]->v1]) + abs(obj_coeff[G->V_links[i]->v2]));
...
void Cplex_Noproj_algo::modify_pricer(list<Constraint*> ct){
...
// return true if a stable set have been found by Cplex
	bool  Cplex_Noproj_algo::find_stableset(StableSet* stable, SCIP_Real& objvalue) {
	...
	if ( !cplex.solve() ) {
		...
		for(int k =0; k < G->nb_links ; k++){
			if( (cplex.getValue(y[k])>1-eps)){
				stable->add(G->V_links[k]->v1);
				stable->add(G->V_links[k]->v2);
...
void Cplex_Noproj_algo::setupBranching(SCIP_ConsData data){
	...
	while(d != NULL){
		...
		if(d->same){
			...
			for(list<C_link*>::iterator it = G->V_nodes[d->v1].L_adjLinks.begin() ; it != G->V_nodes[d->v1].L_adjLinks.end(); it ++){
				Ct1+= y[edgeNumber(d->v1,(*it)->return_other_extrem(d->v1))];
			
			
			
			} 
			for(list<C_link*>::iterator it = G->V_nodes[d->v2].L_adjLinks.begin() ; it != G->V_nodes[d->v2].L_adjLinks.end(); it ++){
				Ct1-= y[edgeNumber(d->v2,(*it)->return_other_extrem(d->v2))];
...
void Cplex_Noproj_algo::setoutBranching(){
