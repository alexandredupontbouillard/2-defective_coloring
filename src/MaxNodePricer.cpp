...
MaxNodePricer::MaxNodePricer(SCIP* scip_, const char* p_name, C_Graph* GG, C_master_coloring * MM,int priority, int nbNodes) : ObjPricerColoring(scip_,p_name,priority,MM){
		G=GG; 
		M=MM;
...
StableSet* MaxNodePricer::coloring_pricing(SCIP* scip, SCIP_RESULT* result){
...
void MaxNodePricer::setupBranching(SCIP_ConsData data){
	setoutBranching();
	A_cplex.setupBranching(data);

}

void MaxNodePricer::setoutBranching(){
