...
void Cplex_pricing_algo::create_MIP(C_Graph *GG,int maxNodes){
	...
	for (k=0;k<G->nb_nodes;k++){
		...
		for(k2= 0; k2 < G->nb_nodes;k2++){
			if((k2 != k)  && (G->adjacency[k][k2] ==1) ){
				cst+= x[k2];
				n_size++;
			}
		} 
...
// return true if a stable set have been found by Cplex
	bool  Cplex_pricing_algo::find_stableset(StableSet* stable, SCIP_Real* objvalue) {
...
void Cplex_pricing_algo::setupBranching(SCIP_ConsData data){
...
void Cplex_pricing_algo::setoutBranching(){
